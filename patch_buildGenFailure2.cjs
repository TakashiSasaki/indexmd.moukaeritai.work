const fs = require('fs');
let lines = fs.readFileSync('src/lib/visualAnalysis/publicSamples/reportBuilder.ts', 'utf8').split('\n');

const startIdx = lines.findIndex(l => l.includes('function buildGenerationFailureSummary(items: PublicSampleBatchRunItem[]) {'));
...
fs.writeFileSync('src/lib/visualAnalysis/publicSamples/reportBuilder.ts', lines.join('\n'));
