...
import { GeminiSdkProviderTransport, getProviderTransport } from "./lib/visualAnalysis/providerTransport";
...
export function createApp(dependencies: AppDependencies = {}) {
  app.set("jobStore", dependencies.jobStore || defaultJobStore);
  app.set("providerTransport", dependencies.providerTransport || getProviderTransport());
  app.set("sampleResolver", dependencies.sampleResolver);
  app.set("idGenerator", dependencies.idGenerator || (() => crypto.randomUUID()));
  app.set("clock", dependencies.clock || { now: () => new Date() });
  app.set("imageFetcher", dependencies.imageFetcher || fetchPublicSampleImage);

  return { app };
}

function getInjectedJobStore(req: express.Request): typeof defaultJobStore {
  return req.app.get("jobStore") || defaultJobStore;
}

function getInjectedProviderTransport(req: express.Request): any {
  return req.app.get("providerTransport") || getProviderTransport();
}

function getInjectedSampleResolver(req: express.Request): SampleResolver | undefined {
  return req.app.get("sampleResolver");
}

function getInjectedImageFetcher(req: express.Request): any {
  return req.app.get("imageFetcher") || fetchPublicSampleImage;
}
...
export async function analyzePublicSample(options: {
  ...
  try {
    ...
    const transport = options.providerTransport || getProviderTransport();
...
app.post("/api/visual/batch-jobs", async (req, res) => {
  try {
    ...
    const store = getInjectedJobStore(req);
    const transport = getInjectedProviderTransport(req);
    const resolver = getInjectedSampleResolver(req);
    const imageFetcher = getInjectedImageFetcher(req);
