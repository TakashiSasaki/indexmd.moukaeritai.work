...
import { ScanCallbacks } from "../lib/drive/types";
import { runScanJob } from "../lib/drive/scanner";
import { runIndexingJob, IndexerCallbacks } from "../lib/drive/indexer";
...
export default function DriveDashboard({ userId, token, config, onUpdateConfig, logs, onAddLog, onClearLogs, onSessionExpiry, activeTab, setActiveTab }: DriveDashboardProps) {
  ...
  const startGenerationJob = async () => {
    ...
    // Pre-calculate children for O(1) lookup during O(N) loop mapping to prevent O(N^2) complexity
    const childrenByParentId = new Map<string, Directory[]>();
