...
These notes must also be completely preserved intact!`);
  ...
  // ⚡ Bolt: Pre-calculate file lists per directory to optimize O(N) rendering performance
  const filesByDir = useMemo(() => {
    const groups: Record<string, any[]> = {};
    for (const item of processedSummaries) {
      const parentIdVal = item.parent_id || item.parentId;
      if (parentIdVal) {
        if (!groups[parentIdVal]) groups[parentIdVal] = [];
  ...
  const generatedIndexMd = useMemo(() => {
    ...
    // Filter file summaries belonging to this directory (O(1) lookup)
    const filesInFolder = filesByDir[selectedDirId] || [];
      ...
      <div className="bg-indigo-950/5 border border-indigo-200/60 p-5 rounded-lg space-y-4">
        ...
        <div className="max-w-md">
          ...
          >
            ...
            {dirs.map((dir) => {
              const fileCount = filesByDir[dir.drive_id]?.length || 0;
              return (
