...
import { startVisualBatchJob, activeRunners } from './jobRunner';
...
test('startVisualBatchJob blocks dispatch after daily quota while preserving in-flight success and pending samples', async () => {
  ...
  assert.equal(blockedJob.items.find(item => item.sampleId === 'sample-B')?.failureKind, 'providerQuotaExceeded');
