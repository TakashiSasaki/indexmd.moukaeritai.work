...
export async function runIndexingJob(
  ...
  // ⚡ Bolt: Pre-calculate children directories grouped by parent_id to optimize O(N^2) lookup to O(N)
  const childrenByParentId = new Map<string, DirectoryMetadata[]>();
  for (const d of directories) {
    if (!d.parent_id) continue;
    let children = childrenByParentId.get(d.parent_id);
    if (!children) {
      children = [];
      childrenByParentId.set(d.parent_id, children);
    }
    children.push(d);
  }
