...
describe('appTabs', () => {
  test('APP_TABS has all required tabs', () => {
    assert.strictEqual(APP_TABS.length, 11);
    ...
    assert.ok(APP_TABS.find(t => t.id === 'image-experiment-client'));
    assert.ok(APP_TABS.find(t => t.id === 'image-experiment-server'));
    assert.ok(APP_TABS.find(t => t.id === 'image-experiment-matrix'));
  });

  test('VALID_TAB_IDS has all ids', () => {
    assert.strictEqual(VALID_TAB_IDS.length, 11);
