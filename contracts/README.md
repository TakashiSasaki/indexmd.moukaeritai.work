...
- **`summary-analysis/`**: Canonical schema for text-derived AI structured summaries (one-line, detailed summary, keywords, subject areas).
- **`text-analysis-record/`**: Envelope wrapping text summaries with execution diagnostics, input details, and quality evaluations.
- **`visual-analysis/`**: Pure computer vision schema capturing element categories, visible labels, context, and textual OCR information from images.
- **`image-analysis-record/`**: Envelope wrapping visual analysis output with resizing policies, technical metadata, quality status, and diagnostics.
- **`public-visual-sample/`**: Regression evaluation schemas (ground truth expected metadata, batch summaries, batch diagnostic reports).
- **`api/`**: Payload specifications for standard API exchange routes (endpoints request/response shapes).
- **`vocabularies/`**: Governed term lists (e.g. `document-kinds`, `subject-domains`, `keyword-sources`).
