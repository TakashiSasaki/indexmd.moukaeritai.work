...
app.post("/api/drive/debug/generate-file-summary", async (req, res) => {
  ...
  try {
    ...
    let cacheKey = "";
    if (mode === "structured") {
      cacheKey = `${fileId}__${safeModel}__${temp}__${tP}__${tK}__mode_${mode}__v${SUMMARY_ANALYSIS_PROMPT_VERSION.replace(/\./g, '_')}__sys_${SUMMARY_DEBUG_SYSTEM_INSTRUCTION_VERSION.replace(/\./g, '_')}__sch_${SUMMARY_ANALYSIS_SCHEMA_VERSION.replace(/\./g, '_')}__${instructionHash}__${modTime}.json`;
    } else {
         ...
         try {
           ...
           if (mode === "structured") {
             try {
               ...
               const validationErrors = getSummaryAnalysisValidationErrors(normalized);
