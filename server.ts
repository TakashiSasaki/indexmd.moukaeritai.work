...
import { getModelCapability, getStructuredExecutionMode, getVisualModelCapability, validateModelExecution, getModelExecutionPolicy } from "./src/lib/modelCapabilities";
import { processStructuredSummaryOutput } from "./src/lib/summaryAnalysis/serverUtils";
import { buildTextAnalysisRecordFromStructuredSummaryResult, buildFailedTextAnalysisRecord } from "./src/lib/summaryAnalysis/recordBuilder";
import { 
...
app.post("/api/visual/batch-jobs", async (req, res) => {
  try {
    ...
    // 1. Validate request-wide configuration (targetSampleIds)
    ...
    // 2. Validate modelName against model execution policy
    const modelVal = validateModelExecution(modelName);
    if (!modelVal.allowed) {
      if (modelVal.error === "modelDiscontinued") {
        return res.status(400).json({
          error: `Model ${modelName} has been discontinued and is no longer available for new executions.`,
          errorType: "modelDiscontinued"
        });
      } else {
        return res.status(400).json({
          error: `Model ${modelName} is not supported for visual analysis.`,
          errorType: "modelUnsupported"
        });
      }
    }

    // 3. Validate jsonMode / provider schema compatibility
    const cap = getModelCapability(modelName);
    if (jsonMode === "native_schema" && !cap.supportsNativeResponseSchema) {
      return res.status(400).json({
        error: `Model ${modelName} does not support native JSON schema mode.`,
    ...
    // Cancel any existing active jobs to ensure a fresh start ONLY after preflight passes
