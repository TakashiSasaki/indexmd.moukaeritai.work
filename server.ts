...
app.post("/api/drive/debug/generate-file-summary", async (req, res) => {
  ...
  const { fileId, modelName, temperature, topP, topK, customInstruction } = req.body;
  if (!fileId) {
  ...
  try {
    ...
    const cacheKey = `${fileId}__${safeModel}__${temp}__${tP}__${tK}__v${PROMPT_SPEC_VERSION.replace(/\./g, '_')}__${instructionHash}__${modTime}.json`;
    ...
    if (cachedSnippet) {
      ...
      if (mimeType === "application/pdf" || mimeType.startsWith("image/")) {
         ...
         try {
           ...
           const filePrompt = buildDebugBinaryFileSummaryPrompt({
             name: fileMeta.name,
    ...
    const filePrompt = buildDebugTextFileSummaryPrompt({
      name: fileMeta.name,
      mimeType: fileMeta.mimeType,
      contentSample: contentSample
    }, customInstruction);
