...
3. Read this file.
4. Inspect the latest target branch, normally `chatgpt`.
...
Do not rely on `main` as durable storage for ChatGPT changes because Google AI Studio may force-push it.
...
- Treat `chatgpt` as the durable target for ChatGPT-authored work.
- Do not depend on `main` retaining ChatGPT commits.
- Protect `.github/workflows/sync-main-to-jules-integration.yml` unless the user explicitly changes branch workflow policy.
