...
- **`jules`**: The branch used by Google Jules for audits, refactors, and maintaining cross-agent standards.
- **`automation/sync-main-to-jules`**: The fallback branch used by GitHub Actions if `main` cannot be cleanly fast-forwarded into `jules`.

## 2. CI/CD (GitHub Actions)
- **Sync Workflow**: `.github/workflows/sync-main-to-jules.yml` automatically triggers on pushes to `main`. It attempts to merge `main` into `jules`. If conflicts occur, it opens a PR. Do NOT delete or disable this workflow.
...
- If the sync workflow breaks, a human or agent must manually merge `main` into `jules` resolving conflicts while respecting `AGENTS.md` constraints.
