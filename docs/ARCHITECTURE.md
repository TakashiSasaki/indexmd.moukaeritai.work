...
- **Gemini Engine**: Resilient retry wrapper with exponential backoff and model fallback chains to handle quota constraints and model unavailabilities gracefully.
...
- **Model Fallbacks**: If `gemini-3.5-pro` fails (Quota/Not Found/Rate Limit), the engine automatically falls back to `gemini-3.1-pro-preview` and then `gemini-3.5-flash` to ensure session continuity.
...
## 5. Storage & Safety Boundaries
- **Google Drive API**: Interacts directly with Drive. Does NOT run full Drive-wide indexing by default. Does NOT delete files or folders.
- **Token Storage**: Drive access tokens are short-lived and stored in memory/sessionStorage. Refresh tokens are NOT stored on the client. LocalStorage is NOT used for tokens.
- **Cache & History**: Snippets and summaries are cached in `cache/`. AI experiment history is stored in `cache/experiment-history/experiment_history.json`. The `cache/` directory must NOT be committed. Raw model output text is not persisted by default to protect PII.
- **Firestore**: Uses database ID `indexmd-db` (not `(default)`). Metadata sync updates are batched.
- **Contract-Centric Schema Validation**: All structured outputs are strictly validated against canonical schemas governed under `contracts/` before persistence.
